...
from app.schemas.user_schema import User, UserCreate, UserPatch, UserUpdate
...
def validate_new_user(user_in: UserCreate) -> UserCreate:
    """Valida el body del POST y que el correo no esté registrado."""
    _ensure_email_available(user_in.email)
    return user_in


def validate_user_replacement(
    user_in: UserUpdate, user: User = Depends(get_user_or_404)
) -> UserUpdate:
    """Valida el body del PUT y que el correo no sea de otro usuario."""
    _ensure_email_available(user_in.email, user.id)
    return user_in


def validate_user_changes(
    user_in: UserPatch, user: User = Depends(get_user_or_404)
) -> dict[str, Any]:
